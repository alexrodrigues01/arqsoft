...
class ReviewControllerTest {
    ...
    void createReview() {
        ...
        ReviewDTO expectedReview = new ReviewDTO(5L, reviewText,date,"Aprovado","Engraçado",rating,10);
    ...
    void putAcceptRejectReview() {
        ...
        LocalDate date = LocalDate.of(2023,2,2);
        ReviewDTO expectedReview = new ReviewDTO(reviewID, "reviewText",date,approved,"Engraçado",2.0,10);
    ...
    void getRecomendedReviews() {
        ...
        Review review = new Review(1L,1L,"approved","ReviewText"
                , List.of(), List.of(),"Report", LocalDate.MAX,"Funfact",product,rating,user);
        when(reviewRecommendation.getReviewsRecommended(1L)).thenReturn(List.of(review));

        ResponseEntity<List<ReviewDTO>> actual = reviewController.getRecomendedReviews(1L);

        assertEquals(200, actual.getStatusCodeValue());
        assertEquals(ReviewMapper.toDtoList(List.of(review)), actual.getBody());
    }
}