...
class ReviewController {
    ...
    @Autowired
    public void setReviewRecommendation(@Value("${review.recommendation}") String bean, ApplicationContext applicationContext){
        reviewRecommendation = (ReviewRecommendation) applicationContext.getBean(bean);
    }

    @Autowired
    public void setService(@Value("${review.service}") String bean, ApplicationContext applicationContext){
        rService = (ReviewService) applicationContext.getBean(bean);
    }
    ...
    public ResponseEntity<List<ReviewDTO>> getRecomendedReviews(@PathVariable(value = "userId") final Long userId){

        List<Review> r = reviewRecommendation.getReviewsRecommended(userId);
        System.out.println(r);
        return ResponseEntity.ok().body(ReviewMapper.toDtoList(r));
    }
